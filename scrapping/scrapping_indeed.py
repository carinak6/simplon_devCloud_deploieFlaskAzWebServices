...
class Indeed:
    ...
    def recupereSalaire(self, donnes):
        ...
        combien= donnes.count('Salaire')
        #traitement de text pour avoir le salaire avec les donnes envoye
        if combien >1:
            try:
                # recherche la dernière occurrence de la valeur spécifiée 
                position= donnes.rindex('Salaire')
            except:
                print('Salaire ne se trouve pas dans la description')
                logging.error('Salaire ne se trouve pas dans la description')
        else:
        ...
        print('salaire retourné : '+ salaire)
